...
wprx_i18n["ru"]["server.nl4.name"] = "Нидерланды 2 (Амстердам)";
wprx_i18n["en"]["server.nl4.name"] = "Netherlands 2 (Amsterdam)";
...
wprx_i18n["ru"]["server.se1.name"]  = "Швеция (Стокгольм)";
wprx_i18n["en"]["server.se1.name"]  = "Sweden (Stockholm)";

var wprx_srvs = ['nl3', 'nl4', 'ru2', 'ru3', 'us3', 'us4', 'uk2', 'fr2', 'se1'];
...
'    <div id="wprx-hideme" style="position: absolute; right: 6px;top:108px;"><a onclick="document.getElementById(\'wprx-wrapper\').style.display=\'none\'; window.wprx_top_panel.setHideCookie(\'ANOHIDECOOKIE\', 1); return false;" href="#"><img src="'+RQ_PATH + '/images/hide2.png" /> '+this.i18n('link.hide')+'</a></div>' +
...
'		    <button class="wprx-button" onclick="var zdenurl = document.getElementById(\'wprx-urltext\').value;  window.location = \'' + RQ_PATH + '/cmd?urlText=\'+encodeURI(zdenurl);return false">'+this.i18n('button.go')+'</button>' +
'		  </div>' +
'		</fieldset>' +
'	      </form>' +
'	      <ul class="wprx-menu">' +
...
wprx_jq(function() {
    ...
    if(wprx_top_panel.init()) {
        wprx_jq('#wprx-page #wprx-ipselect').on('change', function(){
            window.location = 'http://' + this.value + '/home287/cmd?urlText='+ encodeURI(REAL_URL);
            return false;
        });
    }
}); 
