...
function calc_anonym(){
  var sections={"primary":0,"secondary":0};
  var result=0;
  failed_anon=[];
...
function get_os_full(){
  ...
  for(var i=1;i<os_container.length;i++){
    if(os_container[i- 1]==os_container[i]||os_container[i]==''){os_container.splice(i,1);
    i-=1;
...
function detect_os_from_header(ua){
  ...
  if(!/\(/.test(result))
  result=result.replace(/\)$/,'');
  result=(!result&&macintosh==1)?'Macintosh':result;result=(!result&&windows==1)?'Windows':result;
  var mobile_detected=0,
  in_result=0;
  for(var mid in mobiles){
...
inters['de']=[];
inters['fr']=[];
var en_msg=['Mismatch','Match','Unknown','enabled','disabled','more','hide','show','Cannot connect to whois','Anonymizer detected','Yes','No','May be','cached',"Your anonymity"];var de_msg=['Nichtübereinstimmung','Übereinstimmung','unbekannt','aktiviert','deaktiviert','mehr','ausblenden','anzeigen','Keine Verbindung zu Whois','Anonymizer erkannt','Ja','Nein','Vielleicht','Cache','Ihre Anonymität'];var fr_msg=['Incompatible','Correspond','Inconnu','compatible','invalide','encore','cacher','montrer',"N'arrive pas а se connecter а whois",'Anonymizer détecté','Oui','Non','Peut-être','cache',"Votre anonimat"];
...
function iloc(phrase){
  var lang='en';
  if(document.cookie.indexOf("LANG=ru")>=0||/^\/ru/.test(window.location.pathname)){
    lang='ru';
    
  }
  if(document.cookie.indexOf("LANG=de")>=0||/^\/de/.test(window.location.pathname)){
    lang='de';
    
  }
  if(document.cookie.indexOf("LANG=fr")>=0||/^\/fr/.test(window.location.pathname)){
    lang='fr';
...
function check_time_difference(opts){
  ...
  var system_ts=(Math.round(dts.getTime()/ 1000)+((dts.getTimezoneOffset()*60)*(-1)));
...
function get_window_size(){
  ...
  return win_width+"x"+ win_height+" ("+ screen.width+"x"+ screen.height+")";
  
}
var scripts=new Object;
scripts={
  encolor:'#4b4b4b',anonymizer:function(domain,container){
    if((window.location.hostname!=domain)||window["_proxy_jslib_THIS_HOST"]||(typeof ginf!='undefined')||window["REAL_PROXY_HOST"]){$(container).removeClass('disabled').addClass('alarm').html('WebProxy');
