...
return z;return z.replace(/./g,function(z){return char_to_entity[z]?'&'+ char_to_entity[z]+';':z;});};function calc_anonym(){var sections={"primary":0,"secondary":0};var result=0;failed_anon=[];var list=["tor","anonymizer","proxy","ua_mismatch","time_mismatch","dsbl","flash","java","activex","webrtc_local_net","proxy_ports","track","dns","ips","vpn_provider","langs"];try{for(var i in list){var src=list[i];if(src=="dns"||src=="ips"){for(var i=0;i<pub[src].length;i++){if(pub[src][i]["iso"]=="-"||pub[src][i]["iso"]==""||pub["ip_plain"]["iso"]=="")
...
else if(pub[src]>=1){result+=scores[src][0];if(src!='track'){sections[scores[src][1]]++;}
failed_anon.push(src);}}}catch(e){;}
...
var inters=new Array;inters['ru']=[];inters['de']=[];inters['fr']=[];var en_msg=['Mismatch','Match','Unknown','enabled','disabled','more','hide','show','Cannot connect to whois','Anonymizer detected','Yes','No','May be','cached',"Your anonymity"];var de_msg=['Nichtübereinstimmung','Übereinstimmung','unbekannt','aktiviert','deaktiviert','mehr','ausblenden','anzeigen','Keine Verbindung zu Whois','Anonymizer erkannt','Ja','Nein','Vielleicht','Cache','Ihre Anonymität'];var fr_msg=['Incompatible','Correspond','Inconnu','compatible','invalide','encore','cacher','montrer',"N'arrive pas а se connecter а whois",'Anonymizer détecté','Oui','Non','Peut-être','cache',"Votre anonimat"];var ru_msg=['Не совпадает','Совпадает','Неизвестно','включено','отключено','еще','скрыть','показать','Невозможно соединиться с whois','Обнаружен анонимайзер','Да','Нет','Возможно','кэш',"Ваша анонимность"];for(var i in en_msg){inters['ru'][en_msg[i]]=ru_msg[i];inters['de'][en_msg[i]]=de_msg[i];inters['fr'][en_msg[i]]=fr_msg[i];}
function iloc(phrase){var lang='en';if(document.cookie.indexOf("LANG=ru")>=0||/^\/ru/.test(window.location.pathname)){lang='ru';}
if(document.cookie.indexOf("LANG=de")>=0||/^\/de/.test(window.location.pathname)){lang='de';}
if(document.cookie.indexOf("LANG=fr")>=0||/^\/fr/.test(window.location.pathname)){lang='fr';}
if(lang=='en'){return phrase;}
else{var result="";result=inters[lang][phrase];if(result==undefined){return phrase;}
else{return result;}}}
function ll(phrase){return iloc(phrase);}
...
var scripts=new Object;scripts={encolor:'#4b4b4b',anonymizer:function(domain,container){if((window.location.hostname!=domain)||window["_proxy_jslib_THIS_HOST"]||(typeof ginf!='undefined')||window["REAL_PROXY_HOST"]){$(container).removeClass('disabled').addClass('alarm').html('WebProxy');$('.anonymizer-status').addClass('main-bad');return 1;}
return 0;},js:function(el){$(el).removeClass('disabled').html(ll("enabled"));$(el+'-icon').addClass('enabled');},java:function(el){if(window.navigator.javaEnabled()){$(el).removeClass('disabled').html(ll("enabled"));$(el+'-icon').addClass('enabled');return 1;}
return 0;},activex:function(el){try{var AXO=ActiveXObject;$(el).removeClass('disabled').html(ll("enabled"));$(el+'-icon').addClass('enabled');return 1;}
...
if(RTCPeerConnection){$(el).removeClass('disabled').html(ll("enabled"));$(el+'-icon').addClass('enabled');$('#webrtc-expand-block .item').show();return 1;}
else{$('#webrtc-expand-block').addClass('non-ex');}
return 0;}
catch(e){$('#webrtc-expand-block').addClass('non-ex');};},vbscript:function(el){try{VBSEnabled();$(el).removeClass('disabled').html(ll("enabled"));$(el+'-icon').addClass('enabled');return 1;}
catch(e){};},firebug:function(el){try{if(window.console&&(window.console.firebug||window.console.exception)){$(el).removeClass('disabled').html(ll("enabled"));$(el+'-icon').addClass('enabled');return 1;}
else{return 0;}}
catch(e){};},adblock:function(el){try{if(isAdBlockEnabled){$(el).removeClass('disabled').html(ll("enabled"));$(el+'-icon').addClass('enabled');return 1;}
else{return 0;}}
catch(e){return 1;};},flash:function(el){var _flash_installed=0;try{_flash_installed=((typeof navigator.plugins!="undefined"&&typeof navigator.plugins["Shockwave Flash"]=="object")||(window.ActiveXObject&&(new ActiveXObject("ShockwaveFlash.ShockwaveFlash"))!=false));if(_flash_installed){$(el).removeClass('disabled').html(ll("enabled"));$(el+'-icon').addClass('enabled');return 1;}
else{return 0;}}
...
return 0;}
function set_ports_data(el,data){try{if(typeof(data)!='object'){data=$.parseJSON(data);}}catch(e){};var opened=[];var rules=['suspicious','proxy','corporate'];if(typeof(data)=='object'){for(var pid=0;pid<data['opened'].length;pid++){var pushed=0;var port=data['opened'][pid];for(var rule in rules){if(data['rules'][rules[rule]][port]==1){opened.push('<span class="'+ rules[rule]+'">'+ port+'</span>');pushed++;if(rules[rule]=='proxy'){pub["proxy_ports"]=1;if(pub["proxy"]<=1){$('.proxy-status').addClass('main-bad');$('.proxy-status-message').html('<span class="dashed">'+ll('Yes')+"</span>");$('.proxy-status-message').addClass('alarm');$('.proxy-status-message span').on('click',function(){$('#tab-ext').click();$('html, body').animate({scrollTop:$('#ports-block').offset().top},400);event.preventDefault();});}}}}
if(pushed==0){opened.push(port);}}
if(opened.length!=0){$(el).html(opened.join(", ")+(data['cached']?(' ['+ iloc('cached')+']'):''));}
else{$(el).html(iloc('No'));}
calc_anonym();}}
